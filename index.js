...
if (searchInput) {
 searchInput.addEventListener('input', (e) => {
   ...
   const filtered = q
     ...
     : GAMES_JSON; // empty query = show all
   deleteChildElements(gamesContainer);
...
const companyMsg = `
 A total of <strong>${totalGames}</strong> games have been listed on our platform.
 ${numUnfunded === 0
   ? "All games are fully funded 🎉"
   : `<strong>${numUnfunded}</strong> ${numUnfunded === 1 ? "game remains" : "games remain"} unfunded—consider helping them out!`}
`;


// create a new DOM element containing the template string and append it to the description container
const desc = document.createElement('p');
desc.innerHTML = companyMsg;
...
topEl.innerHTML = `${topGame.name} — $${topGame.pledged.toLocaleString()}`;
firstGameContainer.appendChild(topEl);


// do the same for the runner up item
const secondEl = document.createElement('p');
secondEl.innerHTML = `${runnerUp.name} — $${runnerUp.pledged.toLocaleString()}`;
